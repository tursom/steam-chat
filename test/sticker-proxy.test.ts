...
test('legacy sticker proxy names resolve through official metadata and real misses remain 404', async t => {
  ...
  const service = createChatService({
    ...
    fetchImpl: async (value: RequestInfo | URL) => {
      ...
      if (url.hostname === 'api.steampowered.com') {
        return Response.json({ response: { definitions: [definition], count: 1, total_count: 1 } });
      }
      if (decodeURIComponent(url.pathname).endsWith('/sticker/Cat Cam talking')) {
  ...
  assert.ok(requested.some(url => url.includes('/sticker/Cat%20Cam%20talking')));
