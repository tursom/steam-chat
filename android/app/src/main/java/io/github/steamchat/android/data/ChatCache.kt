...
class ChatCache(context: Context) : SQLiteOpenHelper(context, File(context.noBackupFilesDir, "chat.sqlite").absolutePath, null, 1) {
        ...
        try {
            ...
            for (i in 0 until items.length()) {
                ...
                val values = ContentValues().apply {
                    ...
                    put("semantic_id", Protocol.semanticIdentity(item))
    ...
    fun containsEvent(scope: String, eventId: String): Boolean = readableDatabase.rawQuery("SELECT 1 FROM messages WHERE scope=? AND event_id=? LIMIT 1", arrayOf(scope, eventId)).use { it.moveToFirst() }
